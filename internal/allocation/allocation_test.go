...
func subTestGetPermission(t *testing.T) {
	...
	foundPermission1 := a.GetPermission(addr.String())
	assert.Equal(t, p, foundPermission1)

	foundPermission2 := a.GetPermission(addr2.String())
	assert.Equal(t, p2, foundPermission2)

	foundPermission3 := a.GetPermission(addr3.String())
	assert.Nil(t, foundPermission3, "should be nil if not added.")
}
...
func subTestRemovePermission(t *testing.T) {
	...
	foundPermission := a.GetPermission(p.Addr.String())
	assert.Equal(t, p, foundPermission, "Got permission should be same as added.")
